...
clean_by_time() {
    ...
    if ! date -d "$start_time"; then
        echo -e "\e[91mError: Invalid start time format\e[0m"
        exit 1
    fi
    if ! date -d "$end_time"; then
    ...
    find /home -type d | while read -r dir; do
        if [[ "$(basename "$dir")" =~ ^[a-zA-Z]+_[0-9]{6}$ ]]; then
            dir_ctime=$(stat -c %Y "$dir")
...
clean_by_mask() {
    ...
    find /home -type d | while read -r dir; do
        ...
        if [[ "$basename" =~ ^${letters_mask}[a-zA-Z]*_[0-9]{6}$ ]]; then
